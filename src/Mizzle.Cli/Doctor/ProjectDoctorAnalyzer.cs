...
internal static class ProjectDoctorAnalyzer
{
    public static ProjectDoctorReport Analyze(ProjectDoctorInfo info)
    {
        ...
        if (warnOnConfiguration && Regex.IsMatch(code, @"\b(?:Text|Varchar|Char|Integer|BigInt|Boolean|Uuid|Date|Timestamptz|Identity|NVarChar|NVarCharMax|VarChar|Int|Bit|UniqueIdentifier|DateTime|DateTime2|Timestamp)\s*\(\s*@?[A-Za-z_]", RegexOptions.Multiline))
        {
