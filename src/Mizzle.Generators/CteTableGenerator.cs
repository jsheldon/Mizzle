...
public sealed class CteTableGenerator : IIncrementalGenerator
{
    ...
    private static void Generate(SourceProductionContext context, ImmutableArray<CteTableSite> sites)
    {
        ...
        foreach (var group in valid)
        {
            ...
            if (hasNamespace)
            {
                sb.Append("namespace ").AppendLine(group.Key.Namespace);
                sb.AppendLine("{");
            }

            var indent = hasNamespace ? "    " : "";
            sb.Append(indent).Append("public sealed class ").Append(group.Key.TypeName)
                .Append(" : ").Append(baseType).Append('<').Append(group.Key.TypeName).AppendLine(">");
            sb.Append(indent).AppendLine("{");
            sb.Append(indent).Append("    public ").Append(group.Key.TypeName).Append("() : base(")
                .Append(SymbolDisplay.FormatLiteral(shape.CteName, quote: true)).AppendLine(") { }");
            sb.AppendLine();
            foreach (var column in shape.Columns)
            {
                sb.Append(indent).Append("    public ").Append(columnType).Append('<').Append(column.ClrTypeName).Append("> ")
        ...
        context.AddSource("Mizzle.CteTables.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
    }
