...
public sealed class SchemaGenerator : IIncrementalGenerator
{
    ...
    private static void Generate(SourceProductionContext context, TableModel table, bool trimStrings)
    {
        ...
        stringBuilder.AppendLine(table.Namespace);
        stringBuilder.AppendLine("{");
        stringBuilder.Append("    public sealed record ");
        stringBuilder.Append(table.Singular);
        stringBuilder.Append('(');
        stringBuilder.Append(string.Join(", ", table.Columns.Select(c => MemberType(c) + " " + c.Name)));
        stringBuilder.AppendLine(");");

        var insertables = table.Columns.Where(c => !c.IsIdentity).ToList();
        stringBuilder.Append("    public sealed record New");
        stringBuilder.Append(table.Singular);
        ...
        {
            stringBuilder.Append('(');
            stringBuilder.Append(string.Join(", ", insertables.Select(c => MemberType(c) + " " + c.Name)));
            stringBuilder.AppendLine(");");
        }

        stringBuilder.Append("    public static class ");
        stringBuilder.Append(table.TableName);
        stringBuilder.AppendLine("Mapper");
        stringBuilder.AppendLine("    {");
        stringBuilder.Append("        public static ");
        stringBuilder.Append(table.Singular);
        stringBuilder.AppendLine(" Read(global::System.Data.Common.DbDataReader r)");
        stringBuilder.Append("            => new(");
        stringBuilder.Append(string.Join(", ", table.Columns.Select((c, i) => ReadCall(c, i, trimStrings))));
        stringBuilder.AppendLine(");");
        stringBuilder.AppendLine("    }");
        stringBuilder.AppendLine("}");

        var hint = table.Namespace + "." + table.TableName + ".Mizzle.g.cs";
        context.AddSource(hint, SourceText.From(stringBuilder.ToString(), Encoding.UTF8));
    }
